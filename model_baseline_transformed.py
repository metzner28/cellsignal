...
# %%
data = CellSignalDataset('../md_final.csv')
...
val, train = train_test_split(pd.read_csv("../md_final.csv"), test_size = TRAIN_VAL_SPLIT, random_state = 4242)
train.to_csv("../baseline_tx_train.csv", index = False)
val.to_csv("../baseline_tx_val.csv", index = False)

train = CellSignalDataset("../baseline_tx_train.csv", transform = torchvision.transforms.RandAugment())
val = CellSignalDataset("../baseline_tx_train.csv")
...
df_model.to_csv("baseline_tx/baseline_tx_ce.csv", index = False)

torch.save(model.state_dict(), 'baseline_tx/baseline_tx_ce.pt')
