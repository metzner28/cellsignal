...
df_train = df %>%
  ...
  mutate(batch_idx = 1 + floor(train_row / 10 - 0.1)) %>%
...
sirna_sample = c(738,438,679,248,368,595,237,494,123,564)
df_train = df_train %>%
  filter(sirna_id %in% sirna_sample)
df_val = df_val %>%
  filter(sirna_id %in% sirna_sample)
...
write.csv(df_train, "md_contrastive_train_10views_10classes.csv", row.names = F)
write.csv(df_val, "md_contrastive_val_10views_10classes.csv", row.names = F)
