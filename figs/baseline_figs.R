library(tidyverse)
library(ggplot2)
setwd("~/Dropbox/Documents/tri-i/FoDS/project/analysis/221118_results/")

df_test = read_csv("../../cellsignal/md_test_final.csv") %>%
  select(-c(1,2))
df_pred = read_csv("baseline_ce_preds_top5.csv")
...
ggsave("../../final/cell_type_dist.pdf", device = 'pdf', width = 9, height = 6)

train_path = read_csv("baseline_ce.csv") %>%
  mutate(epoch = 1:32,
...
ggsave("../../final/training_path.pdf", device = 'pdf', width = 10, height = 3)
...
ggsave("../../final/well_type_dist.pdf", device = 'pdf', width = 9, height = 6)
...
ggsave("../../milestone/pclass_dist.pdf", device = 'pdf', width = 10, height = 5)
